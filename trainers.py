...
class Trainer():
    ...
    def train(self):
        ...
        for inputs, targets in tqdm(self.train_dataloader):
            ...
            if batch_loss.dim() == 0:
                self.batch_losses.append(batch_loss.data.unsqueeze(0))
            else:
                self.batch_losses.append(batch_loss.data)
            if batch_metric.dim() == 0:
                self.batch_metrics.append(batch_metric.data.unsqueeze(0))
            else:
        ...
        for val_inputs, val_targets in self.val_dataloader:
            ...
            if val_batch_loss.dim() == 0:
                self.val_batch_losses.append(val_batch_loss.data.unsqueeze(0))
            else:
                self.val_batch_losses.append(val_batch_loss.data)
            if val_batch_metric.dim() == 0:
                self.val_batch_metrics.append(val_batch_metric.data.unsqueeze(0))
            else:
